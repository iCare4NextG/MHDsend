...
public class MHDsend {
	...
	public static void main(String[] args) {
		...
		opts.addOption(null, "time", true, "Time(seconds) (default : 30)");
		...
		opts.addOption("r", "reference-id", true, "For referenceId");
		...
		try {
			...
			if (cl.hasOption("h") || args.length == 0) {
				...
				formatter.printHelp(
					"MHDsend [<options>]",
					"\nSend documents in IRM", opts,
					"Examples: $ ./MHDsend --document-status");
				System.exit(0);
			}
			//time
			if (cl.hasOption("time")) {
				time = cl.getOptionValue("time");
				optMap.put("time", time);
			} else {
				optMap.put("time", time);
			}
			logger.info("o server_url = {}", server_url);
			//Server-url
			...
			if (cl.hasOption("event")) {
				...
				for(String tmpEvent : eventArr) {
					if (!checkCode(tmpEvent)) {
						error = true;
						logger.error("Event Error = {}", tmpEvent);
						break;
					} else {
			...
			if (cl.hasOption("content-type")) {
				...
				error = true;
				logger.error("content-type Error : {}", cl.getOptionValue("content-type"));
			}
			//patient-id
			if (cl.hasOption("patient-id")) {
				logger.info("patient-id = {}", cl.getOptionValue("patient-id"));
			...
			if (cl.hasOption("data-binary")) {
				...
				error = true;
				logger.error("data-binary Error : {}", cl.getOptionValue("data-binary"));
			}
			//manifest-title
			...
			if (cl.hasOption("document-title")) {
				...
				logger.info("getOptionValue(data-binary) : {}", cl.getOptionValue("data-binary"));
			...
			if (cl.hasOption("reference-id")) {
				...
				for(String referenceId : referenceArr) {
					...
					if (!checkReferenceId(referenceId)) {
						error = true;
						logger.error("reference-id Error : {}", referenceId);
					} else {
